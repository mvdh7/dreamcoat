from great_circle_calculator.__conversion import (
    _point_to_radians,
    _point_to_degrees,
    _radians_to_degrees,
    _degrees_to_radians,
...
def distance_between_points(p1, p2, unit="meters", haversine=True):
    ...
    if haversine:
        ...
        a = sin(d_lat / 2) * sin(d_lat / 2) + cos(lat1) * cos(lat2) * sin(
            d_lon / 2
        ) * sin(d_lon / 2)
        a = min(1, a)
...
def midpoint(p1, p2):
    ...
    lat3 = atan2(
        sin(lat1) + sin(lat2), sqrt((cos(lat1) + b_x) * (cos(lat1) + b_x) + b_y * b_y)
    )
...
def point_given_start_and_bearing(p1, course, distance, unit="meters"):
    ...
    lon2 = lon1 + atan2(
        sin(brng) * sin(delta) * cos(lat1), cos(delta) - sin(lat1) * sin(lat2)
    )
